...
**Interval KDTree**
- Continuar modificando funciones que usan ndsh2 por nds3 (kd-tree)
	- cutting point
	- distance
- Pensar estrategia que vaya arreglando los nodos a medida que se insertan/eliminan

Usar árbol para cálculo de distancia
![image](https://i.imgur.com/EGc1zeq.png)
**Búsqueda en árbol**

Arreglo de distancias por cada dimensión d (inicialización con ceros)
me interesa encontrar el máximo para cada distancia
y luego el mínimo valor del arreglo

  

Caja inicial: región dominada por p=lb de la caja

Descarte: si subárbol es subóptimo en todas las coordenadas: range[i].ub() - p[i] < d[i]

di se actualiza si node[i]-p[i] > d[i]

mantener ub[i] global

contractar con: di < min(ub[i])

Selección: caja que maximiza ub[i] con i=argmin(ub[i])
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzUxNTM5ODE3LDEzMzMyNTc2MTEsLTE4OT
Y3NTM3MjQsLTQxMTU1ODYwNSwtMTUzMzE0NzMzNCwtMTgyNDAz
ODAxMCwxMjUzNDI4MzUxLDIxMjE1MTI5MDIsODYxOTY1MDg5LD
E0MjQ1MTE0NDEsMTQyMzI5NTY3NCwtMTQwMzY5NzgyOSwtMTIx
NTY0ODA5OSwtMTMwNzQzNzc3NSw5Mjk4OTY0OTIsLTE4NTE4ND
c1MTIsLTIxMDA0MzMyMSw3OTE2Nzg3NDAsMTQ2ODE3MjAzMCw3
ODg5MjEwNTRdfQ==
-->