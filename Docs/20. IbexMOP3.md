...
- Continuar modificando funciones que usan ndsh2 por nds3 (kd-tree)
	- cutting point
	- distance
- Reemplazar ndsh2 por nds3 y correr instancias paper.
- Correr instancias con 3 dimensiones.	
...
Arreglo de distancias por cada dimensión d (inicialización con ceros) me interesa encontrar el máximo para cada distancia y luego el mínimo valor del arreglo
...
* d[i] se actualiza si node[i]-p[i] > d[i]
* mantener ub[i] global
* reducir con: di <= min(ub[i])
* Selección: caja que maximiza ub[i] con i=argmin(ub[i])
* distancia = min(d[i])
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI3MTkzMTUzNSwtMTMwNDI3OTM2OSwtMj
UyOTQyMzIxLC0xMzUxNzQxMDQ3LDEzMzMyNTc2MTEsLTE4OTY3
NTM3MjQsLTQxMTU1ODYwNSwtMTUzMzE0NzMzNCwtMTgyNDAzOD
AxMCwxMjUzNDI4MzUxLDIxMjE1MTI5MDIsODYxOTY1MDg5LDE0
MjQ1MTE0NDEsMTQyMzI5NTY3NCwtMTQwMzY5NzgyOSwtMTIxNT
Y0ODA5OSwtMTMwNzQzNzc3NSw5Mjk4OTY0OTIsLTE4NTE4NDc1
MTIsLTIxMDA0MzMyMV19
-->