...
````python
def contract(y, cy, Dbox):
    # x + y + z + 10 = 0
    # x + y_lb + z +10 = 0
    
    dim = len(y)
    for i in range(dim):
        # punto inicial (f1(y2,y3), y2.lb, y3.lb)
        p0 = copy.deepcopy(y.lb())
        p0[i] = -(np.dot(cy[0:dim],p0)+cy[dim] - cy[i]*y_lb[i])/cy[i]
        
        #considerar planos que dominen a p0
        
        # deriv (-a1, a2, 0, 0,...); (-a1, 0, a2, 0, 0,..)
        deriv = np.zeros(dim)
        deriv[i] = cy[i]
        for j in range(dim):
            if i==j: continue
            deriv[j]=-cy[j]
            t=min_t(p0,deriv,Dbox[2])
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzgwNzY1NTI0LC03NTgyODA1MzcsLTExMD
Q5NTIwNjEsODAxNzM4NDI2LC0xNDE5MTk4NDA2LC0xNzQ2MTQz
NDAzLDM2MjU0NjkxOSwtODM4NjcyNDMwLDU0NTUwOTgzOSwxNT
UwNjE5Njc3LC04MTgyMzY5NzYsLTE3NjI3NTE5NDZdfQ==
-->