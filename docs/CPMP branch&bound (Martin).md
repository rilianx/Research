...
def validate2(seq, st, sd):
	...
	h0 = h
    for soo, sdd in seq: # vuelve en la secuencia
       h[soo]+=1; h[sdd]-=1
       if h[sdd]<h0[sdd]: variant_stacks.insert(sdd)
       if sdd==s_t andif h[st]==h0[st] and h[sd]==h0[sd]: 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIwOTA0NTIxNiwtMjA5ODk2NzgwNiwtMT
