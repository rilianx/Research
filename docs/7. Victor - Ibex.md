...
2. Si la diferencia de tamaños es menor a un threshold se usa el PA y Pb (misma linearización). Sólo se llama Gauss-Seidel y se contracta hasta punto-fijo. Contracción GS óptima.
4. 
   


5. Se decide si se continua con P (comparando el tamaño de la caja). Si no cumple se lineariza y se recalcula P.
6. Si se decida linearizar, se actualiza A y se recalcula P, PA y PB. 
7. GS normal (smart propagation).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQwNjU0MDI0NiwtODQ5OTI4MTY0LDQ4MT
